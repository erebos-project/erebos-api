...
set API_PACKAGE_PLACE_SHARED=0
...
:set_compress_list
	...
	if "%API_PACKAGE_PLACE_README%" == "1" (
		copy /Y %API_TREE_MAIN_DIRECTORY%\%API_README% %API_PACKAGE_NAME_PATTERN%\
	)

	if "%API_PACKAGE_PLACE_LICENSE%" == "1" (
		copy /Y %API_TREE_MAIN_DIRECTORY%\%API_LICENSE% %API_PACKAGE_NAME_PATTERN%\
	)

	mkdir %API_PACKAGE_NAME_PATTERN%\%API_PACKAGE_LIBRARY_DIRECTORY%

	if "%API_PACKAGE_PLACE_AR%" == "1" (
		copy /Y %API_TREE_MAIN_DIRECTORY%\%API_LIBRARY_AR_EREBOS% %API_PACKAGE_NAME_PATTERN%\%API_PACKAGE_LIBRARY_DIRECTORY%
	)

	if "%API_PACKAGE_PLACE_SHARED%" == "1" (
		copy /Y %API_TREE_MAIN_DIRECTORY%\%API_LIBRARY_SHARED_EREBOS% %API_PACKAGE_NAME_PATTERN%\%API_PACKAGE_LIBRARY_DIRECTORY%
	)

	if "%API_PACKAGE_PLACE_HEADERS%" == "1" (
		mkdir %API_PACKAGE_NAME_PATTERN%\%API_PACKAGE_INCLUDE_DIRECTORY%
		copy /Y %API_TREE_MAIN_DIRECTORY%\%API_HEADER_PATTERN% %API_PACKAGE_NAME_PATTERN%\%API_PACKAGE_INCLUDE_DIRECTORY%
	)
